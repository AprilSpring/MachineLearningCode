...
def dataProcessing(path, train_name='round1_ijcai_18_train_20180301.txt', 
    ...
    #参数说明：
    #featureExtract 是否提取新的统计特征
    #featureDrop 是否对某些不重要的特征进行删除
    #missingValue 是否进行补缺失值
    #choice_date 是否对数据集做日期筛选，如果True，则需要确定choose_date参数
    ...
    if featureExtract == True:
        ...
        train_integate = pd.DataFrame()
        for d in Counter(train_pd['date']).keys():
            print(d)
            train_pd2 = train_pd[train_pd['date'] == d]
            
            train_user_feature = userFeature(data=train_pd2, user_list=train_user) 
            train_shop_feature = shopFeature(data=train_pd2, shop_list=train_shop) 
            train_item_feature = itemFeature(data=train_pd2, item_list=train_item)     
                     
            train_integate1 = integrate_feature(data=train_pd2, user_feature=train_user_feature, 
                                      shop_feature=train_shop_feature, item_feature=train_item_feature)           
            
            train_integate = pd.concat([train_integate,train_integate1], axis=0)
    ...
    def type_conversion(data):
        ...
        data['user_occupation_id'] = data['user_occupation_id'].astype(str)
    ...
    if featureDrop == True:
        ...
        for i in f1_list:
            ...
            p.append(levene(train_pd[i],test_pd[i]).pvalue) #方差齐性检验：检验数据分布
            #p.append(f_oneway(train_pd[i],test_pd[i]).pvalue) #方差分析
        for ii in f2_list:
            p.append(ranksums(train_pd[ii],test_pd[ii]).pvalue) #离散特征
        
        tmp1 = pd.DataFrame(p,index=f_list) 
        tmp2 = tmp1[tmp1.iloc[:,0] < 0.0001]
        drop_f1 = list(tmp2.index)
        
        
        #2、通过lightgbm和xgboost的特征重要性（手动添加）
        #'context_page_id','user_gender_id','context_id','shop_id','user_id','user_occupation_id'
        drop_f2 = []
        #'user_id',
    ...
    if missingValue == True:
        ...
        def missValue(data):
            for i in data.columns:
                if type(data[i].iloc[0]) == np.int64 and len(data[i][data[i].isin([-1])])>0:
                    data[i][data[i].isin([-1])] = np.median(data[i])
                if type(data[i].iloc[0]) == int and len(data[i][data[i].isin([-1])])>0:
                    data[i][data[i].isin([-1])] = np.median(data[i])
                if type(data[i].iloc[0]) == str and len(data[i][data[i].isin(['-1'])])>0:   
                    data[i][data[i].isin(['-1'])] = str(np.ceil(np.median(pd.DataFrame(list(Counter(data[i]).keys())).astype(int))))
                else:
                    pass
            return data
    ...
    #最终去掉不加入的特征
    drop_id = ['context_timestamp','date','instance_id','shop_id',
               'item_id','user_id','item_brand_id','context_page_id',
               'context_id','time','user_occupation_id','user_gender_id']
    drop_id2 = drop_id + ['is_trade']
    test_feature = test_pd.drop(drop_id,axis=1).values
    fnames = train_pd.drop(drop_id2,axis=1).columns
    train_feature = train_pd.drop(drop_id2,axis=1).values
...
def trainClassifierLGBM(x_train,y_train,x_test,y_test,test_feature,test_index,f_names=None):
    ...
    watchlist=[lgb_test]
    lgbm = lgb.train(params=params, train_set=lgb_train, num_boost_round=600,
    ...
    prob = lgbm.predict(test_feature, num_iteration=-1) #lgbm.best_iteration
...
def trainClassifierXGB(x_train,y_train,x_test,y_test,test_feature,test_index,f_names=None,buffer=False):
    ...
    param = {'max_depth':3,'eta':0.03,'min_child_weight':1, 
    ...
    bst = xgb.train(params=plst,dtrain=dtrain,num_boost_round=300,evals=watchlist, 
...
def fiveFoldCV(train_pd,train_feature,train_label,test_feature,test_index,method,f_names,date_split=True):       
    ...
    if method == 'lightGBM': 
        if date_split == True:
            d = sorted(list(Counter(train_pd['date']).keys()))
            ...
            for i in range(0,5):
                [x_train1,x_test1,y_train1,y_test1] = dataSplit(
                        ...
                        f_names=f_names,date_split=True,train_d=list(d[i:i+2]),test_d=list(d[i+2:i+3]))
                ...
                test_date = d[i+2:i+3]
                tmp = pd.concat([pd.DataFrame(x_test1),pd.DataFrame(test_date*test1_prob.shape[0]),pd.DataFrame(test1_prob),pd.DataFrame(y_test1)],axis=1)
    ...
    test_date = ['2018-09-25'] #最终预测的日期
    test_pred2 = pd.DataFrame(np.mean(test_pred, axis=1))
    test_pred2 = test_pred2.reset_index(drop=True) 
    test_feature_add = pd.concat([pd.DataFrame(test_feature),
                                  pd.DataFrame(test_date*test_pred.shape[0]),test_pred2],axis=1) #按行求均值
...
def fusionModel(train_pd,train_feature,train_label,test_feature,
    ...
    ## add: predict prob to 0/1
    #b1 = train_pd_add[train_pd_add['is_trade']==1]
    #prob_mean = np.mean(b1['predicted_score'])
    #train_pd_add['predicted_score'][train_pd_add['predicted_score'] >= prob_mean] = 1
    #train_pd_add['predicted_score'][train_pd_add['predicted_score'] < prob_mean] = 0
    
    fnames_add = train_pd_add.drop(['date','is_trade'],axis=1).columns
    train_feature_add = train_pd_add.drop('is_trade',axis=1).values
    train_label_add = train_pd_add.is_trade.values
    
    #导入添加预测值的test set
    testFile_add = './result_fusion/'+ method1 + '_test_feature_add.csv'
    test_pd_add = pd.read_csv(testFile_add, header=0, sep=' ')
    
    ## add: predict prob to 0/1
    #test_pd_add['predicted_score'][test_pd_add['predicted_score'] >= prob_mean] = 1
    #test_pd_add['predicted_score'][test_pd_add['predicted_score'] < prob_mean] = 0
    
    test_feature_add = test_pd_add.drop(['date'],axis=1).values
    ...
    x_train.columns = train_pd_add.columns.drop(['is_trade'])
    x_train = x_train.drop(['date'],axis=1).values
    
    x_test = pd.DataFrame(x_test)
    x_test.columns = test_pd_add.columns
...
def main():
    ...
    lgbm_prob.to_csv(('./result/submit_lightGBM.csv'), index=True, index_label='instance_id', header=['predicted_score'], sep=' ')
    #0.08341
    ...
    result = 0.3 * xgboost.iloc[:,1] + 0.2 * lightGBM.iloc[:,1] + 0.2 * lightGBM_xgboost.iloc[:,1] + 0.3 * xgboost_lightGBM.iloc[:,1]
    ...
    #result = pd.read_csv('./result/submit_integrate_20180330_160701.txt', header=0, sep=' ')
